/**
 * NOK Chat Overlay - same card opens on Home, Shorts, Upload, Settings, Video
 * Include this script on every page that has #chat-overlay and .chat-trigger
 */
(function() {
    ...
    if (resizeHandle && card) {
        ...
        document.addEventListener('mousemove', function(e) {
            ...
            var newW = Math.max(280, Math.min(window.innerWidth * 0.95, startW + (e.clientX - startX)));
            var newH = Math.max(400, Math.min(window.innerHeight * 0.9, startH + (e.clientY - startY)));
